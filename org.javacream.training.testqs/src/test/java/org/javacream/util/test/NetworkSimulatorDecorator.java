 ...
 * <ul>
 * <li>Makes a deep copy of all parameters using serialization. Therefor
 * parameters are passes call-by-value.</li>
 * <li>Makes a deep copy of results or exception objects using serialization.
 * Therefor results and exceptions are passes call-by-value.</li>
...
public class NetworkSimulatorDecorator implements InvocationHandler {
	...
	public static <T> T decorate(T toDecorate, long delay) {
		ClassLoader classLoader = NetworkSimulatorDecorator.class.getClassLoader();
		Class<?>[] interfaces = toDecorate.getClass().getInterfaces();
		NetworkSimulatorDecorator decorator = new NetworkSimulatorDecorator();
